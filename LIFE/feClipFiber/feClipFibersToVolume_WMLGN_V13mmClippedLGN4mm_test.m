...
for i = 2%:length(subDir)
    ...
    for k= 1:length(fgfile)
        ...
        switch k
            case 1 % for left OR
                NOTroiFile =  roiname{2};                
                MaskroiFile = roiname{1};
            case 2 % for right OR
                NOTroiFile = roiname{1};                
                MaskroiFile = roiname{2};
        end
       
        % load roi
        cd(roiDir)
        NOTroi = dtiReadRoi(NOTroiFile);       
        Maskroi = dtiReadRoi(MaskroiFile);
        %%
        % load fg
        cd(fgDir)    
        fg=fgRead(fgfile{k});
...
%         
        ...
        fg1.name = sprintf('%s_BWhiteMatter.mat',fg1.name);
