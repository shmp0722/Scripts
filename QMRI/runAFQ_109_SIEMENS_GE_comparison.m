function [afq, patient_data, control_data, norms, abn, abnTracts]  = runAFQ_109_SIEMENS_GE_comparison
%%
...
!flirt -in T1_map_lsq_SIEMENS.nii.gz -ref t1.nii.gz -out T1_map_lsq_SIEMENS_aligned_T1w.nii.gz -omat inSIEMENSrefT1w.mat
!flirt -in T1_map_lsq_GE.nii.gz -ref t1.nii.gz -out T1_map_lsq_GE_aligned_T1w.nii.gz -omat inGErefT1w.mat
...
% SO_/SMNS_T1map     ='/biac4/wandell/biac2/wandell/data/DWI-Tamagawa-Japan/QuantitativeT1/SO_tmp_vibe/SPGR_1/Align_2.0312_2.0312_2/T1_map_lsq.nii.gz';
...
SO_SMNS_T1map     = '/biac4/wandell/biac2/wandell/data/DWI-Tamagawa-Japan/QuantitativeT1/SO_GE_SIEMENS_comparison/T1_map_lsq_SIEMENS_aligned_T1w.nii.gz';
...
SO_GE_T1map       = '/biac4/wandell/biac2/wandell/data/DWI-Tamagawa-Japan/QuantitativeT1/SO_GE_SIEMENS_comparison/T1_map_lsq_GE_aligned_T1w.nii.gz';
...
[afq, patient_data, control_data, norms, abn, abnTracts] =  AFQ_run_forHH(sub_dirs, sub_group, afq);
save AFQ_GE_SIEMENS_cimparison_t1w afq
...
%     Y2 = afq.vals.T1_map_lsq{ii}(2,:);
    ...
    Y1 = afq.vals.T1_map_lsq_SIEMENS_aligned_T1w{ii}(1,:);
    Y2 = afq.vals.T1_map_lsq_SIEMENS_aligned_T1w{ii}(2,:);
...
%     plot(X,Y3,'Color',c(3,:),'linewidth',3);
    ...
    % save plot 
    print(gcf, '-dpng',sprintf('SIEMENS_GE_Comparison %s.png',afq.fgnames{ii}))
...
for ii = 1:20;
    figure(ii);hold on;
    X = 1:100;
    Y1 = afq.vals.T1_map_lsq_SIEMENS_alignedGE{ii}(1,:);
    Y2 = afq.vals.T1_map_lsq_SIEMENS_alignedGE{ii}(2,:);
