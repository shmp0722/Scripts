function run_V1RoiCutEccentricity_NumFiber_20k(MakeEccROI,contrackgen,cleanfiber,outlier)
%
% outlier = {'AFQ','mba'}
%
%% idnetify ID
...
ctrParams.projectName = 'V1eccentricity_NumFiber_20k';
...
%                 switch outlier
%                     case 'mba'
                        % remove both shorter and longer
                        [fgclean, ~] = mbaRemoveFiberLengthOutliers(fgOut1,3,'both',[]);
                        % based on distance from teh core
                        numberOfNodes = 100;
                        maxSD = 3;
                        [fgclean,coordsDistribution] = mbaRemoveFibersCoordsDistributionOutlier(fgclean,numberOfNodes,M,maxSD);
%                     case 'AFQ'
%                         % Remove outlier fibers
%                         maxDist = 3;  maxLen = 3;   numNodes = 100;
%                         M = 'mean';
%                         count = 1;  show = 1;
%                         [fgclean2 ,keep] =  AFQ_removeFiberOutliers(fgOut1,maxDist,maxLen,numNodes,M,count,show);
%                 end
...
%     roiDir{i},'/dwi_2nd/ROIs');% should change
    dt6    = fullfile(homeDir,subDir{i},'/dwi_2nd/dt6.mat');
    dt6    = dtiLoadDt6(dt6);
    direction ='AP';
    Nodes  = 100;
