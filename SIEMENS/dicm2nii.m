...
% DICOM2NII converts Siemens dicom files into NIfTI or img/hdr files. 
% 
...
% 
% If no input, or any of the first two inputs is empty, a graphic user
% interface will appear.
% 
% If the first input is a zip file, such as those downloaded from dicom
% server, DICOM2NII will extract files into a temp folder, create NIfTI files
% into the data folder, and then delete the temp folder. For this reason,
% it is better keep the zip file as a backup.
% 
% If a folder is the data source, DICOM2NII will convert all files in the
...
%        Change filename from dcm2nii.m to reduce confusion from MRICron.
%        GUI implemented into a single file.

% Todo: remove dependency on dicominfo dicomread

if nargin && ischar(src) && strcmp(src, 'dicm2nii_gui_cb')
    feval(dataFolder); % mis-use for GUI
    return;
end
...
elseif iscellstr(src) % multiple files
    dcmFolder = fullfile(getfield(what(fileparts(src{1})), 'path'));
    n = length(src);
    fnames = src;
    for i = 1:n
        foo = dir(src{i});
...
disp('dicm2nii for Siemens MR by Xiangrui.Li@gmail.com');
...
fprintf('Elapsed time by dicm2nii.m is %.1f seconds\n\n', toc);
...
%% Subfunction: return true if the file is a dicom file.
function tf = isdicm(fname)
fid = fopen(fname, 'r','ieee-le');
if fid<0, tf = false; return; end
fseek(fid, 128, 'bof');
sig = fread(fid, 4, '*char');
fclose(fid);
tf = strcmp(sig', 'DICM'); % may miss some dicom
...
%% Subfuction: create GUI or bring GUI to front
function dicm2nii_gui
hs = findobj('Type', 'figure');
i = find(strcmp('dicm2nii_fig', get(hs, 'tag')));
if ~isempty(i), figure(hs(i)); return; end % bring to front if exists

scrSz = get(0, 'ScreenSize');
fig1 = figure('toolbar', 'none', 'menubar', 'none', 'resize', 'off', ...
    'tag', 'dicm2nii_fig', 'Position', [200 scrSz(4)-500 420 300], ...
    'name', 'DICOM to NIfTI Converter', 'numbertitle', 'off', ...
    'WindowButtonUpFcn','dicm2nii(''dicm2nii_gui_cb'',''mouseup_callback'');');
clr = get(fig1, 'color');
...
uicontrol('Style', 'edit', 'position', [88 220 296 24],...
    'FontSize', 9, 'horizontalalignment', 'left', 'background', 'white', ...
    'tag', 'src', 'string', '', 'TooltipString', str, ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''set_src'');');
uicontrol('Style', 'pushbutton', 'position', [384 221 24 22], 'tag', 'browseSrc', ...
    'fontsize', 9, 'string', '...', 'TooltipString', 'Browse dicom source', ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''fileDialog'');');
...
uicontrol('Style', 'edit', 'position', [88 180 296 24],...
    'FontSize', 9, 'horizontalalignment', 'left', 'background', 'white', ...
    'tag', 'outFolder', 'string', '', 'TooltipString', str, ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''set_outFolder'');');
uicontrol('Style', 'pushbutton', 'position', [384 181 24 22],...
    'fontsize', 9, 'string', '...', 'TooltipString', 'Browse result folder', ...
    'tag', 'browseDst', ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''fileDialog'');');

str = 'Choose output file format';
uicontrol('Style', 'text', 'position', [10 130 90 30],...
    'FontSize', 9, 'horizontalalignment', 'left', ...
    'string', 'Output format', 'Background', clr, 'TooltipString', str);
uicontrol('Style', 'popup', 'background', 'white', 'tag', 'rstFmt', ...
    ...
    'Compressed multiple files (.img.gz / .hdr.gz)'], 'value', 2, ...
    'Position', [88 134 320 30], 'TooltipString', str);

str = 'Choose the way to deal with MoCo series';
uicontrol('Style', 'text', 'position', [10 90 90 30],...
    'FontSize', 9, 'horizontalalignment', 'left', ...
    'string', 'MoCoSeries', 'Background', clr, 'TooltipString', str);
uicontrol('Style', 'popup', 'background', 'white', 'String', ...
    [' Use both original and MoCo series|' ...
     ' Ignore MoCo series if both present|' ...
     ' Ignore original series if both present'], 'value', 2, ...
    'tag', 'mocoOpt', 'Position', [88 94 320 30], 'TooltipString', str);

str = 'Enter subject ID only if the data contains more than one subjects';
uicontrol('Style', 'popup', 'background', clr, 'tag', 'subjPop', ...
    'String', ' Source is for a single subject| Convert only for subject', ...
    'Position', [10 50 210 30], 'TooltipString', str, ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''pop_subj'');');
uicontrol('Style', 'edit', 'position', [224 57 184 24],...
    'FontSize', 9, 'horizontalalignment', 'left', 'background', 'white', ...
    'tag', 'subjName', 'string', '', 'TooltipString', str);

uicontrol('Style', 'pushbutton', 'position', [120 10 200 30], ...
    'fontsize', 9, 'string', 'Start conversion', ...
    'tag', 'convert', 'Enable', 'off', ...
    'callback', 'dicm2nii(''dicm2nii_gui_cb'',''do_convert'');');

hs = guihandles(fig1); % get handles
guidata(fig1, hs); % store handles
return;

%% Subfuction for GUI: start conversion
function do_convert %#ok<*DEFNU>
hs = guidata(gcbo);
src = get(hs.src, 'string');
if get(hs.srcType, 'value') == 3
    src = get(hs.src, 'UserData');
end
outFolder = get(hs.outFolder, 'string');
...
set(hs.convert, 'enable', 'off', 'string', 'Converting ...'); 
drawnow;
dicm2nii(src, outFolder, rstFmt, mocoOpt, subjName);
try set(hs.convert, 'enable', 'on', 'string', 'Start conversion'); end %#ok

%% Subfuction for GUI: select source type
function popSource
hs = guidata(gcbo);
i = get(hs.srcType, 'value');
txt = {'Dicom folder' 'Zip file' 'Dicom files'};
...
%% Subfuction for GUI: browse src file or result folder
function fileDialog
h = gcbo;
hs = guidata(h);
folder = fileparts(get(hs.src, 'string'));
if ~isdir(folder), folder = pwd; end

if h == hs.browseDst % result folder
    dst = uigetdir(folder, 'Select a folder to save data files');
    if isnumeric(dst), return; end
    set(hs.outFolder, 'string' , dst);
    mouseup_callback;
    return;
end
...
i = get(hs.srcType, 'value');
if i == 1 % folder
    src = uigetdir(folder, 'Select root folder containing DICOM files');
...
%% Subfuction for GUI: set source file or folder
function set_src
h = gcbo;
drawnow;
str = get(h, 'string');
if ~exist(str, 'file')
    ...
    if isempty(val)
        ...
        set(h, 'string', val);
        errordlg('Invalid file', 'Error Dialog');
        return;
    end
    str = {val.name};
    str = fullfile(folder, str);
end
set(h, 'UserData', str);
pop_subj;
mouseup_callback;

%% Subfuction for GUI: set result folder
function set_outFolder
h = gcbo;
drawnow;
str = get(h, 'string');
if ~exist(str, 'file'), mkdir(str); end
drawnow;
if ~exist(str, 'file')
    set(h, 'string', '');
    errordlg('Invalid result folder', 'Error Dialog');
end
mouseup_callback;

%% Subfuction for GUI: subject option
function pop_subj
hs = guidata(gcbo);
if get(hs.subjPop, 'value')==2 && ~isempty(strtrim(get(hs.subjName, 'string')))
    return;
end
src = get(hs.src, 'UserData');
if iscellstr(src)
...
elseif isdir(src)
    fname = dir(src);
    fname([fname.isdir]) = [];
    fname = fullfile(src, fname(1).name);
else
    fname = src;
end
try %#ok
    s = dicominfo(fname);
    set(hs.subjName, 'string', s.PatientName.FamilyName);
end

%% Subfuction for GUI: enable/disable the start button
function mouseup_callback
hs = guidata(gcbo);
drawnow;
if isempty(get(hs.src,'string')) || isempty(get(hs.outFolder,'string'))
    set(hs.convert, 'enable', 'off');
else
