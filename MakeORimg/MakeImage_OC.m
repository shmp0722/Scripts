...
for ii = 1:length(subs)
    ...
    ROIfile = {'Optic-Chiasm.mat'};
    
    for ij = 1:length(ROIfile)
        %             fg = dtiLoadFiberGroup(fullfile('fibers',ROIfile{ij}));
        %             Roi = dtiReadRoi(fullfile('fibers',ROIfile{ij}));
        Roifile = fullfile('ROIs',ROIfile{ij});
        ...
        img_coords = unique(floor(mrAnatXformCoords(inv(dt.xformToAcpc), Roi.coords)), 'rows');
