function  VolumeCheck_OT_clipped
    ...
    for j = 1:2;  % RH or LH        
        ...
        switch j
            case 1
                ...
                ROI2 = dtiReadRoi(fullfile(baseDir, subjectDir{i}, '/dwi_2nd/ROIs',roiF{2}));
            case 2
                ...
                ROI2 = dtiReadRoi(fullfile(baseDir, subjectDir{i}, '/dwi_2nd/ROIs',roiF{3}));
        end
        ...
        for ij = 1:length(Ids)
            ...
            if length(fg_ori.fibers) <3,
                ...
                if length(fgOut.fibers) < 2
                    Volume{ij} = zeros(nodes,1);
                    nFiber(ij) =length(fgOut.fibers);
                else
                    fg = dtiResampleFiberGroup(fgOut,nodes,[]); % default is number of nodes 100
...
title('R-Optic Tract clipped','fontName','Times','fontSize',14);
axis([0, 1.5, 0, 4500]);
...
% "fig.png"
cd /biac4/wandell/biac2/wandell/data/DWI-Tamagawa-Japan/AFQ_results/Volume_OT_5K
...
print('-r0','-dpng','ROT_clipped_Volume_SE_2.png');
%%
...
% xlabel('conTrack score thresh','fontName','Times','fontSize',12);
% ylabel('Mean Volume [mm^3]','fontName','Times','fontSize',12);
% title('partial R-Optic Tract','fontName','Times','fontSize',14);
% hold off;

%% L-OTS (whole)
Y1 = nanreplace(cell2mat(sCRD.tL(:)));
Y2 = nanreplace(cell2mat(sLHON.tL(:)));
Y3 = nanreplace(cell2mat(sCtl.tL(:)));
...
ylabel('Mean Volume [mm^3]','fontName','Times','fontSize',12);
title('L-Optic Tract clipped','fontName','Times','fontSize',14);
...
pos(4)=height;
set(gcf,'Position',pos);
print('-r0','-dpng','LOT_clipped_Volume_SE.png');
...
pos(4)=height;
set(gcf,'Position',pos);
print('-r0','-dpng','LOT_clipped_Volume_SE_2.png');
%% number of fibers
%R-OR nFiber(whole)
...
CRD  =  nanreplace(vertcat(LOT.FA.mean{j,[2,4,7,8,9]}));
