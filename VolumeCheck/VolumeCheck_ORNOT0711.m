function  VolumeCheck_ORNOT0711
%%
...
for j = 1:length(X)
    for i = 1:length(subjectDir);
        ...
        total_volume{j,i} =  (total_volumeR{j,i} + total_volumeL{j,i})/2;
    end
end
...
%% calculate averaged volume in each group
for j = 1:length(X)
    % CRD
    % Sum
    sCRD.B{j} = sum(horzcat(total_volume{j,[2,4,7,8,9]}))/(5-sum(horzcat(total_volume{j,[2,4,7,8,9]})==0));
    
    % std
    sCRD.B_sd{j} = nanstd(horzcat(total_volume{j,[2,4,7,8,9]}));
    
    %S.E.
    sCRD.B_se{j} = sCRD.B_sd{j}/sqrt(5-sum(horzcat(total_volume{j,[2,4,7,8,9]})==0));
    
%     sCRD.nFiber{j} = sum(horzcat(total_nFiberR{j,[2,4,7,8,9]}))/(5-sum(horzcat(total_nFiberR{j,[2,4,7,8,9]})==0));
    

 % JMD
    % Sum
    sJMD.B{j} = sum(horzcat(total_volume{j,[1,3,5,6]}))/(5-sum(horzcat(total_volume{j,[1,3,5,6]})==0));
    
    % std
    sJMD.B_sd{j} = nanstd(horzcat(total_volume{j,[1,3,5,6]}));
    
    %S.E.
    sJMD.B_se{j} = sJMD.B_sd{j}/sqrt(5-sum(horzcat(total_volume{j,[1,3,5,6]})==0));
    
%     sJMD.nFiberR{j} = sum(horzcat(total_nFiberR{j,[1,3,5,6]}))/(5-sum(horzcat(total_nFiberR{j,[1,3,5,6]})==0));
    
    % LHON
    sLHON.B{j} = sum(horzcat(total_volume{j,10:15}))/(6-sum(horzcat(total_volume{j,10:15})==0));
    
    % std
    sLHON.B_sd{j} = nanstd(horzcat(total_volume{j,10:15}));
    %S.E.
    sLHON.B_se{j} = sLHON.B_sd{j}/sqrt(6-sum(horzcat(total_volume{j,10:15})==0));
    
%     sLHON.nFiber{j} = sum(horzcat(total_nFiberR{j,10:15}))/(6-sum(horzcat(total_nFiberR{j,10:15})==0));
    
    % Ctl
    sCtl.B{j} = sum(horzcat(total_volume{j,16:23}))/(8-sum(horzcat(total_volume{j,16:23})==0));
    
    % std
    sCtl.B_sd{j} = nanstd(horzcat(total_volume{j,16:23}));
    % S.E.
    sCtl.B_se{j} = sCtl.B_sd{j}/sqrt(8-sum(horzcat(total_volume{j,16:23})==0));
    
    sCtl.nFiberR{j} = sum(horzcat(total_nFiberR{j,16:23}))/(8-sum(horzcat(total_nFiberR{j,16:23})==0));
end
%% plot Averaged both hemi
...
% legend('CRD', 'LHON', 'Ctl');
% add error bar
errorbar(X,Y1,cell2mat(sCRD.B_se(:)),'Color',c(3,:))
errorbar(X,Y2,cell2mat(sLHON.B_se(:)),'Color',c(4,:))
errorbar(X,Y3,cell2mat(sCtl.B_se(:)),'Color',[0.5,0.5,0.5])
% errorbar(X,Y4,cell2mat(sCRD.B_se(:)),'Color',c(2,:))


xlabel('conTrack score thresh','fontName','Times','fontSize',12);
ylabel('Mean Volume [mm^3]','fontName','Times','fontSize',12);
title('B-Optic Radiation','fontName','Times','fontSize',14);
% axis([0,1.5,0, 160000])

hold off;

%% save the figure in .png
cd /biac4/wandell/biac3/wandell7/shumpei/matlab/git/LHON/6LHON_9JMD_8Ctl/VolumeComparison/OR_CRD_LHON

print(gcf,'-r0','-dpng','B-OR_Volume_CRD_LHON.png');
print(gcf,'-r0','-depsc','B-OR_Volume_CRD_LHON.eps');


%% plot Averaged both hemi JMD CRD Ctl
...
%% save the figure in .png
cd /biac4/wandell/biac3/wandell7/shumpei/matlab/git/LHON/6LHON_9JMD_8Ctl/VolumeComparison/OR_JMD_CRD

print(gcf,'-r0','-dpng','B-OR_Volume_JMD_CRD.png');
print(gcf,'-r0','-depsc','B-OR_Volume_JMD_CRD.eps');
