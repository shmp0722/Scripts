function fibersPDB = SO_mrtrix_track_between_rois_OCF
%
...
% 1. We load two ROIs in the brai, for Shumpey's project for example we
...
for i = 1:length(subjectDir);
    ...
    fibersFolder = fullfile(baseDir, subjectDir{i}, '/dwi_2nd/fibers/mrtrix_OCF');
    roisFolder   = fullfile(baseDir, subjectDir{i}, '/dwi_2nd/ROIs');
    
%     saveDir = fullfile('/azure/scr1/shumpei/DWI-Tamagawa-Japan',subjectDir{i});
    
    
    % We want to track the cortical pathway (LGN -> V1/V2 and V1/V2 -> MT)
    
    %     for hemi = 1:2 % R = 1, L= 2;
    fromRois = 'lh_V1_smooth3mm_lh_V2_smooth3mm_NOT';
    toRois   = 'rh_V1_smooth3mm_rh_V2_smooth3mm_NOT';
    ...
    % seeding
    %         wm = {'WMmask'};
    wmMaskName      = fullfile(baseDir,  subjectDir{i}, '/dwi_2nd/ROIs','WMmask');
    ...
    files = mrtrix_init(dtFile,lmax,fibersFolder,wmMaskMifName);
    
    % We use a nifti ROI to select the portion of the White matter to use for
    % seeding
    %
    
    fromRoisName      = fullfile(roisFolder,fromRois);
    ...
    %
    toRoisName      = fullfile(roisFolder,toRois);
    ...
    roi{1} = fullfile(roisFolder, fromRois);
    roi{2} = fullfile(roisFolder, toRois);
    ...
    %% Set file names
    roiName         = fullfile(baseDir, subjectDir{i}, '/ROIs/',ni.fname);
...
%     cd(saveDir);
    ...
    [fibersPDB, ~, ~] = mrtrix_track_roi2roi(files, [roi{1} '.mif'], [roi{2} '.mif'], ...
        seedRoiMifName, wmMaskMifName, trackingAlgorithm{1}, ...
        nSeeds, nFibers);
end
return
%% exclude unneccesary fiber by fiber 3D distribution
...
%     fibersFolder = fullfile(baseDir, subjectDir{i}, '/dwi_2nd/fibers/mrtrix_OCF');
    ...
    fg =  fgRead(fibersPDB);
    roi = dtiReadRoi(fullfile(roisFolder,'fs_CC.mat'));
    
    fg = dtiIntersectFibersWithRoi([],'and',[],roi,fg);
    fgWrite(fg,fg.name,'pdb')
%     [~, weights] = mbaComputeFibersCoordsDistribution(fg, 30, []);
%     hist(weights);
    maxSD = 8;
    [fgOut,~] = mbaRemoveFibersCoordsDistributionOutlier(fg,[],[],maxSD);
%     hist(coordsDistribution)
    fgOut.name = sprintf('%s%d','OCF_mbaSD_', maxSD);
